...
// Usar build standalone do PDFKit (fontes embutidas, sem acesso a FS)
...
async function generatePdfBuffer(b: Budget): Promise<Buffer> {
  return await new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.5);
    const startX = 50;
    const widths = { desc: 280, qty: 60, unit: 80, total: 80 } as const;
    ...
    // Total (duas colunas com coordenadas explícitas para evitar sobreposição)
